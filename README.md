...
| 做什么 | 命令 |
|--------|------|
| **前后端一起跑（推荐，`npm run dev` 默认即此项）** | `npm run dev` 或 `npm run dev:all` |
| 只跑前端（Vite） | `npm run dev:fe` |
...
开发时 Vite 会把 **`/admin-api` 动态代理**到实际后端端口（默认读 `backend/.dev-port`）；根目录 `npm run dev` 会先等后端就绪再起前端。**若 3000 已被占用**，后端会自动尝试 3001–3009 并写入 `.dev-port`，无需手改代理。
